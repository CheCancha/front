...
import type { Complex, Schedule, Court, Image } from "@prisma/client";

type FullComplexData = Complex & {
...
export default function ImageSettings({
  ...
  // Memoizamos las imágenes para separar la principal del resto
  ...
  const handleImageUpload = async (
    ...
    try {
      ...
      if (newImages.length > 0) {
        setData((prev) => {
          if (!prev) return null;
          // Si no hay imagen principal antes, la primera nueva será la principal
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Esta es la imagen principal que verán los usuarios al buscar tu complejo.
        </p>
        <div className="mt-4 w-full h-64 rounded-lg bg-gray-200 flex items-center justify-center border">
            {primaryImage ? (
                ...
                <span className="text-gray-500">No hay una imagen de portada seleccionada</span>
            )}
        </div>
      </div>
