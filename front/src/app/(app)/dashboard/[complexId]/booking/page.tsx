...
// Obtenemos la fecha de hoy en formato YYYY-MM-DD para los datos iniciales
const todayStr = new Date().toISOString().split('T')[0];

const initialBookingsData: Booking[] = [
  { id: 101, courtId: 1, customerName: "Juan Pérez", startTime: "18:00", date: todayStr },
  { id: 102, courtId: 3, customerName: "Ana García", startTime: "19:00", date: todayStr },
  { id: 103, courtId: 1, customerName: "Carlos Díaz", startTime: "20:00", date: todayStr },
  { id: 104, courtId: 4, customerName: "María López", startTime: "16:00", date: todayStr },
];
...
interface AddBookingModalProps {
    ...
    courts: Court[];
    timeSlots: string[];
    selectedDate: Date; // Recibimos la fecha seleccionada del calendario
}

const AddBookingModal: React.FC<AddBookingModalProps> = ({ isOpen, onClose, onSubmit, courts, timeSlots, selectedDate }) => {
    // Helper para formatear un objeto Date a un string "YYYY-MM-DD"
    ...
    const [bookingDate, setBookingDate] = useState(formatDateForInput(selectedDate));

    // Efecto para actualizar la fecha del modal si cambia la fecha del calendario principal
    useEffect(() => {
        if (isOpen) {
            setBookingDate(formatDateForInput(selectedDate));
        }
    }, [isOpen, selectedDate]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSubmit({
            ...
            courtId,
            startTime,
            date: bookingDate, // Enviamos la fecha seleccionada
        });
                ...
                >
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="bookingDate" className="block text-sm font-medium text-gray-700 mb-1">Día del Partido</label>
                                <input
                                    id="bookingDate"
                                    type="date"
                                    value={bookingDate}
...
export default function BookingCalendarPage() {
  ...
  const handleAddBooking = (newBookingData: Omit<Booking, 'id'>) => {
    setBookingsData(prev => [
        ...prev,
        { ...newBookingData, id: Date.now() } // Añadimos la nueva reserva con un ID único
    ]);
  };
          ...
          >
            ...
            {timeSlots.map(time => (
              <React.Fragment key={time}>
                ...
                {filteredCourts.map(court => {
                  // Lógica de búsqueda actualizada para incluir la fecha
                  ...
                  return (
                    <div key={`${court.id}-${time}`} className="relative h-16 border-b border-l p-1">
                      {booking ? (
                        ...
                        <button 
                            onClick={() => setIsModalOpen(true)} // También se podría abrir el modal desde aquí
