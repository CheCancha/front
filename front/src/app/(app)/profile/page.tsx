...
export default function ProfilePage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting: isUpdatingPassword }, reset, } = useForm<PasswordFormData>({
  ...
  const filteredBookings = useMemo(() => {
    ...
    switch (bookingFilter) {
      case "Próximas":
        return userBookings.filter((b) => b.status === "CONFIRMADO" && new Date(b.date) >= now);
      case "Completadas":
        return userBookings.filter((b) => b.status === "COMPLETADO");
      case "Historial":
        return userBookings.filter((b) => b.status === "CANCELADO" || b.status === "PENDIENTE" || (b.status === "CONFIRMADO" && new Date(b.date) < now));
      default:
        return userBookings;
    }
  ...
  const handleChangePassword = async (data: PasswordFormData) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(responseData.error || "No se pudo cambiar la contraseña");
  ...
  if (status === "unauthenticated") {
    return <p className="text-center mt-10">Acceso denegado. Por favor, inicia sesión.</p>;
  }
  ...
  return (
    <>
                ...
                <form onSubmit={handleUpdateInfo} className="space-y-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-grow">
                            ...
                            <input id="phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full text-gray-800 bg-transparent border-b focus:outline-none focus:border-brand-orange" readOnly={!isEditingPhone} />
                        </div>
                        <button type="button" onClick={() => setIsEditingPhone(!isEditingPhone)} className="text-gray-500 hover:text-brand-orange">
          ...
          <div className="lg:col-span-2 space-y-8">
            {stats && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <StatCard icon={Trophy} label="Reservas Totales" value={stats.totalBookings} />
                <StatCard icon={Clock} label="Horas Jugadas" value={stats.hoursPlayed} />
                <StatCard icon={Award} label="Reservas Completadas" value={stats.completedBookings} />
              ...
              <div className="border-b border-gray-200 mb-4">
                <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                  {["Próximas", "Completadas", "Historial"].map((tab) => (
                    <button key={tab} onClick={() => setBookingFilter(tab)} className={`${bookingFilter === tab ? "border-brand-orange text-brand-orange" : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"} whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm`}>
                      {tab}
                    </button>
                  ))}
                </nav>
                ...
                <ul className="divide-y divide-gray-200">
                  {filteredBookings.map((booking) => (
                    <li key={booking.id} className="py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div>
                        <p className="font-semibold text-gray-900">{booking.complex}</p>
                        <p className="text-gray-700">{booking.court}</p>
                        <p className="text-sm text-gray-600 flex items-center gap-2 mt-1">
                          <Calendar size={14} />
                          {format(parseISO(booking.date), "eeee dd 'de' MMMM, yyyy", { locale: es })} a las {booking.startTime} hs
                        </p>
                      </div>
                      <div className="flex items-center gap-4">
      ...
      <ModalProfile isOpen={isCancelModalOpen} onClose={() => setIsCancelModalOpen(false)} title="Confirmar Cancelación">
          <div className="p-6">
                  ...
                  <div className="mt-2">
                      {isCancellationTimely ? (
                          <p className="text-sm text-gray-500">
                              Estás cancelando dentro del período permitido. El club será notificado para coordinar la devolución de tu seña.
                          </p>
                      ) : (
                          <p className="text-sm text-gray-500">
                              Estás cancelando fuera del período permitido. Según la política del club, la seña no será reembolsada.
                          </p>
                      )}
                  </div>
              </div>
