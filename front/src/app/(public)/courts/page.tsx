...
import { MapPin, ArrowDownUp, SlidersHorizontal, List, Map, Clock } from 'lucide-react';
...
type Club = {
  ...
  address: string;
  imageUrl: string;
  availableSlots: string[]; // <-- Ahora esperamos los turnos disponibles
};
            ...
            <div className="p-4 flex flex-col flex-grow">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 flex-grow flex flex-col justify-end">
                    {club.availableSlots.length > 0 ? (
                        <>
                            <p className="text-xs font-semibold text-gray-500 mb-2">Próximos turnos hoy:</p>
                            <div className="flex flex-wrap gap-2">
                                {club.availableSlots.map((slot) => (
                                    <div key={slot} className="px-3 py-1 bg-green-100 text-green-800 font-bold rounded-md text-sm">
                                        {slot}
                                    </div>
                                ))}
                            </div>
                        ...
                        {view === "list" ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {isLoading
                                    ...
                                    : complexes.length > 0
                                        ? complexes.map((club) => <ClubCard key={club.id} club={club} />)
                                        : (
