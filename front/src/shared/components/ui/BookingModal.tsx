...
export const BookingModal: React.FC<BookingModalProps> = ({ isOpen, onClose, club, court, time, date }) => {
            ...
            {!preferenceId && (
              <div>
                ...
                {status === 'unauthenticated' && (
                  <div className="mb-4">
                    <label htmlFor="guestName" className="block text-sm font-semibold text-gray-700 mb-1"><User className="inline-block w-4 h-4 mr-1"/> Nombre y Apellido</label>
                ...
                <div className="space-y-4 text-left border-t border-b py-4">
                   <div className="flex items-center gap-3"><Shield className="w-5 h-5 text-brand-orange" /><p><span className="font-semibold">{club.name}</span> - {court.name}</p></div>
                   <div className="flex items-center gap-3"><Calendar className="w-5 h-5 text-brand-orange" /><p>{date.toLocaleDateString("es-AR", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</p></div>
                   <div className="flex items-center gap-3">
                     <Clock className="w-5 h-5 text-brand-orange" />
                     <p>
                        <span className="font-semibold">{time} a {calculateEndTime(time, court.slotDurationMinutes)}hs</span>
                        <span className="text-gray-500 text-sm ml-2">({court.slotDurationMinutes} min)</span>
                     </p>
                   </div>
                   <div className="flex items-center gap-3"><DollarSign className="w-5 h-5 text-brand-orange" /><p>Total a pagar: <span className="font-bold">{court.pricePerHour.toLocaleString("es-AR", { style: "currency", currency: "ARS" })}</span></p></div>
                </div>
                <div className="mt-6">
